...
function parseAppStoreUrl(value) {
  ...
  return { appId: match[1], storefront: STOREFRONT };
}

function validateCollectionInput({ appUrl, country = STOREFRONT, maxReviews = 200 }) {
  const parsed = parseAppStoreUrl(appUrl);
  const requestedCount = Number(maxReviews);

  if (country !== STOREFRONT) {
    throw new ApiError("INVALID_COUNTRY", "本项目仅采集美国 App Store 评论。", { status: 400 });
  }
...
function mapAppleFeed(feedPayload, { appId, fetchedAt }) {
...
function applePageUrl(appId, page) {
  return `https://itunes.apple.com/${STOREFRONT}/rss/customerreviews/page=${page}/id=${appId}/sortby=mostrecent/json`;
}

function cacheFile(cacheDir, appId, page) {
  if (!/^\d+$/.test(appId) || !Number.isInteger(page) || page < 1 || page > MAX_PAGES) {
    throw new ApiError("INVALID_CACHE_KEY", "评论缓存键无效。", { status: 400 });
  }
  return path.join(cacheDir, `${appId}-page-${page}.json`);
}

async function readCache(cacheDir, appId, page, nowMs, ttlMs) {
  try {
    const payload = JSON.parse(await fs.readFile(cacheFile(cacheDir, appId, page), "utf8"));
...
async function writeCache(cacheDir, appId, page, fetchedAt, data) {
  await fs.mkdir(cacheDir, { recursive: true });
  await fs.writeFile(
    cacheFile(cacheDir, appId, page),
...
function toRequestError(error) {
  ...
  return new ApiError("APPLE_REQUEST_FAILED", "无法获取美国 App Store 评论。", {
    status: 502,
    retryable: true,
    cause: error
  });
...
async function requestApplePage({
  ...
  const nowMs = now();
  const cached = await readCache(cacheDir, appId, page, nowMs, cacheTtlMs);
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      const response = await fetchImpl(applePageUrl(appId, page), {
      ...
      if (countAppleEntries(data) > 0) {
        await writeCache(cacheDir, appId, page, fetchedAt, data);
      }
...
async function collectReviews(
  ...
  const { appId, requestedCount } = validateCollectionInput(input);
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    ...
    const pageReviews = mapAppleFeed(result.data, { appId, fetchedAt: result.fetchedAt });
